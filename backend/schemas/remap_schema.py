...
class RemapResult(BaseModel):
    gaps: list[str] | None = Field(
        description=(
            "Identify core required skills, tools, languages, frameworks, and critical responsibilities "
            "from the job description that the candidate has NO demonstrable experience with. "
            "Only include items where there is zero evidence in the candidate's profile. "
            "Output null if there are no gaps. "
            "CRITICAL RULE: If you intend to mention a missing requirement in the 'remap_description', "
            "it MUST be explicitly extracted and listed in this array first."
            "Example: If the JD requires 'Python', 'Docker', and 'managing a team of 5+', but the candidate only knows Python, output ['Docker', 'managing a team of 5+']."
        )
    )
    ...
    remap_description: str = Field(
        description=(
           "A strict, single-sentence explanation focusing on what the candidate is MISSING. "
            ...
            "MUST perfectly match the items you listed in the 'gaps' array. Do not mention a gap here if you did not list it above. "
